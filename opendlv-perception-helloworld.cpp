...
int32_t main(int32_t argc, char **argv) {
	...
	//绘制折线
	cv::polylines(imgWithCones, blueCones_coordiate, false, cv::Scalar(0, 255, 0), 1, 8, 0);
	...
	for (int x = 0; x < 1280; x++)
	{
		...
		points_fitted.push_back(cv::Point(x, static_cast<int>(y)));
	}
	cv::polylines(imgWithCones, points_fitted, false, cv::Scalar(0, 255, 255), 1, 8, 0);
